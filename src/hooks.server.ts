...
const supabase: Handle = async ({ event, resolve }) => {
  ...
  const authGuard: Handle = async ({ event, resolve }) => {
    ...
    if (!event.locals.session && event.route.id?.startsWith('/(authenticated)')) {
      redirect(303, '/registrarse')
    }
    if (event.locals.session && event.route.id?.startsWith('/(unauthenticated)')) {
      redirect(303, '/')
    }
    return resolve(event)
  }
...
export const handle = sequence(setupdDb, supabase, authGuard);
